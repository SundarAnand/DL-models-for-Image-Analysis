...
def ResNet ():
    base_model = applications.resnet.ResNet152(weights= None, include_top=False, input_shape= (256,256,3))
    x = base_model.output
...
with tf.device('/device:GPU:0'):
    ...
    for train_index, test_index in kf.split(X, y):
        ...
        df_output.to_csv("ResNet152_Fold_" + str(count) + ".csv", index=False)
    ...
    df_epochs.to_csv("ResNet152Results.csv", index=False)
