...
def attention(input_size):
    # Define the input as a tensor with shape input_shape
    X_input = Input(input_shape)

    # First Block
    # CONV layer
    conv_1_out = Conv2D(64, (7, 7), strides=(1, 1), activation='relu', padding='same')(X_input)
    # MAXPOOL + BatchNorm
    X = MaxPooling2D((2, 2), strides=2, padding='same')(conv_1_out)
    X = BatchNormalization(axis=-1)(X)

    block_2_in1_conv = Conv2D(64, (3, 3), strides=(2, 2), activation='relu', padding='same')(conv_1_out)
    ...
    # Second Block
    # CONV layer
    conv_2_out = Conv2D(128, (5, 5), strides=(2, 2), activation='relu', padding='same')(attention_input_2)
    # MAXPOOL
    X = MaxPooling2D((2, 2), strides=2, padding='same')(conv_2_out)
    X = BatchNormalization(axis=-1)(X)

    block_3_in2_conv = Conv2D(128, (3, 3), strides=(1, 1), activation='relu', padding='same')(conv_2_out)
    block_3_in2_max = MaxPooling2D((2, 2), strides=2, padding='same')(block_3_in2_conv)

    block_3_in1_conv = Conv2D(128, (3, 3), strides=(2, 2), activation='relu', padding='same')(block_2_in1_max)
    ...
    # Third Block
    conv_3_out = Conv2D(256, (5, 5), strides=(1, 1), activation='relu', padding='same')(attention_input_3)
    # MAXPOOL
...
with tf.device('/device:GPU:0'):
    ...
    for train_index, test_index in kf.split(X, y):
        ...
        df_output.to_csv("Attention_Fold_" + str(count) + ".csv", index=False)
    ...
    df_epochs.to_csv("AttentionResults.csv", index=False)
